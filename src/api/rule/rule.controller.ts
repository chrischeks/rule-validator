...
import { IResponse } from '../../util/rule.interface';
...
export class RuleController extends UniversalsController {
  public baseResponse = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    const { ip, method, originalUrl } = req;
    try {
      const response: IResponse = await new RuleService().processBaseResponse({ ip, method, originalUrl });
